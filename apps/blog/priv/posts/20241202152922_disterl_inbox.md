...
The single mailbox bottleneck compounds that, as the latency between receiving a message and being able to act on it is variable based on the condition of the node mailbox as well as network conditions.

That's not to mention all the issues one faces with:

- [Clock drift](https://en.wikipedia.org/wiki/Clock_drift)
- [Dropped packets](https://en.wikipedia.org/wiki/Packet_loss)
